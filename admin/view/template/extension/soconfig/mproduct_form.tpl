                ...
                <table id="images" class="table table-striped table-bordered table-hover">
                  ...
                  <tbody>
                    ...
                    <tr id="image-row<?php echo $image_row; ?>">
                      ...
                      <td class="text-right"><input type="text" name="product_image[<?php echo $image_row; ?>][sort_order]" value="<?php echo $product_image['sort_order']; ?>" placeholder="<?php echo $entry_sort_order; ?>" class="form-control" />


						  <?php

						  foreach ($product_options as $option){
							echo '<div style="float: left; margin-right: 20px; text-align: left">';
							echo '<strong>' . $option['name'] . '</strong><br/>';

						  	foreach($option['product_option_value'] as $ov){

						  		$val_txt = '';
						  		foreach($option_values[$option['option_id']] as $val){
						  			if ($val['option_value_id'] == $ov['option_value_id']) {
						  				$val_txt = $val['name'];
						  			}
						 		}
								$checked = '';
						  		if (in_array($ov['product_option_value_id'], $product_image['options_selected']) !== false){
						  			$checked = 'checked';
						  		}


						  		echo '<input type="checkbox"
											 name="product_image[' . $image_row .'][options][]"
											 value="' . $ov['product_option_value_id'] . '"
											 id="' . $image_row. '_' . $ov['product_option_value_id'] . '"
											 ' . $checked . '
						  		>'.
						  '<label for="' . $image_row. '_' . $ov['product_option_value_id'] . '" style="font-weight:normal;">' . $val_txt . '</label>'.
						  '<br/>';
