		...
		public function index() {
			...
			if(!$this->checkDatabase()) {
				...
				foreach($results as $result) {
					...
					//var_dump($result['featured_image']);
					if ($result['featured_image']) {
						//$image = HTTP_SERVER . 'image/' . $result['featured_image'];
                        $image = $this->model_tool_image->resize_full(DIR_IMAGE ,  $result['featured_image'], 280, 280);

						$featured_found = 1;

					} else if($result['image']) {
						//$image = HTTP_SERVER . 'image/' . $result['image'];
                        $image = $this->model_tool_image->resize_full(DIR_IMAGE ,  $result['image'], 280, 280);
			...
			if ($simple_blog_article_id) {
				...
				if($article_info) {
					...
					if($article_info['featured_image']) {
                        ...
                        $image = $this->model_tool_image->resize_full(DIR_IMAGE ,  $article_info['featured_image'], 280, 280);
                        $data['image'] = $image;
                    } else if($article_info['image']) {
						//$data['image'] = HTTP_SERVER . 'image/' . $article_info['image'];
                        $image = $this->model_tool_image->resize_full(DIR_IMAGE ,  $article_info['image'], 280, 280);
                        $data['image'] = $image;
