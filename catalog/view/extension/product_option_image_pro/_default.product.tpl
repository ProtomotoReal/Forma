<script type="text/javascript"><!--
	...
	poip_product_default.prototype.setVisibleImages = function(images_param) {
		...
		// << originally theme 422 compatibility, currently common	
			...
			// << SO DESTINO
		} else if ($('ul.thumb-vertical').find('li.owl2-item').find('a').length) {
			var shown_img = [];
			$('ul.thumb-vertical').find('li.owl2-item').each( function(){

				var src = $(this).find('img').src;

				if (($.inArray( src, images) != -1 || $.inArray(decodeURIComponent(src), images) != -1) && $.inArray( src, shown_img) == -1) {
					$(this).show();
					shown_img.push(src);
				} else {
					$(this).hide();
	...
	poip_product_default.prototype.getMainImage = function() {
		...
		var oc2_main_img = $('.large-image img');
		if ( oc2_main_img.length ) { // OC 2.0 default
			return oc2_main_img;
		}
